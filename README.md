...
| cursor-plugin-cc | Claude Code plugin that delegates coding tasks to the Cursor CLI (`cursor-agent`), with Composer 2 as the default execution model. Claude plans and reviews, Cursor executes. Eight `/cursor:*` slash commands plus a `cursor-runner` subagent; jobs run foreground or background. Zero runtime dependencies. | [Github](https://github.com/freema/cursor-plugin-cc) ![GitHub Repo stars](https://img.shields.io/github/stars/freema/cursor-plugin-cc?style=social) | Free (requires Cursor paid plan for Composer 2) |
