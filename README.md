...
| CCHub | Open-source desktop app (Tauri + Rust) for managing the entire Claude Code ecosystem — MCP server marketplace with one-click install, config profile switching between Claude Code/Codex/Gemini CLI/OpenCode, 12 workflow templates, skills & plugins browser, CLAUDE.md visual editor, security audit, and Autopilot task orchestration. Cross-platform, ~20MB binary. | [Github](https://github.com/Moresl/cchub) ![GitHub Repo stars](https://img.shields.io/github/stars/Moresl/cchub?style=social) | Free |
