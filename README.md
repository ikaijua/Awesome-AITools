...
| DvalinCode | Open-source (MIT) local-first coding agent. Supports any OpenAI-compatible model (Ollama, DeepSeek, Claude via OpenRouter) with three trust modes — Chat (read-only), Cowork (plan-then-approve), and autonomous Code. macOS sandboxed shell execution, ~25MB single binary, exposes a REST+WebSocket API as an embeddable agent runtime. | [Github](https://github.com/arthurpanhku/dvalincode) ![GitHub Repo stars](https://img.shields.io/github/stars/arthurpanhku/dvalincode?style=social) | Free |
