...
| CarryCtx | Local-first CLI that gives coding agents persistent memory across sessions, windows, and Git worktrees: structured tasks, progress logs, decisions, and Git-aware checkpoints stored in a single SQLite file per repo. `carryctx resume` restores exactly where the last agent or window left off; full-text search (SQLite FTS5) finds prior work by content. No account, no network access by default, works with any agent (Claude Code, Codex, Copilot, etc.) via CLI or an MCP stdio server. | [Github](https://github.com/Xuepoo/carryctx) ![GitHub Repo stars](https://img.shields.io/github/stars/Xuepoo/carryctx?style=social) | Free |
