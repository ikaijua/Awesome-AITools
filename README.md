...
| VibeKit.bot | Build, deploy, and keep running web apps entirely from your phone. Each app gets its own persistent AI coding agent (Claude, GPT, Gemini, DeepSeek, or a free tier) on a hosted container with a live URL and custom domains, plus one-tap OAuth to 120+ services (Gmail, Slack, Notion, Stripe) so the agent can act on your real accounts. Native iOS app, web dashboard, REST API, CLI, and MCP server. Not affiliated with the unrelated `superagent-ai/vibekit` SDK. | [URL](https://vibekit.bot) | Free/Paid |
