...
| Name | Description | Links | Fees |
| --- | --- | --- | --- |
| Ouroboros | Specification-first Agent OS for AI coding. Runs a Socratic interview first and blocks the immutable Seed spec while ambiguity is above 0.2 unless force is passed explicitly, then drives execution, a layered evaluation gate (Mechanical -> Semantic -> Multi-Model Consensus; all three run on the direct evaluation path, while the evolution loop runs the semantic stage only), and a budgeted evolution loop. When an acceptance criterion defines a `verify_command` or an output assertion, those values are omitted from the worker, so a struggling agent cannot satisfy the assertion string instead of the criterion. Works across 13 runtimes including Claude Code, Codex CLI, Gemini CLI and OpenCode, and ships an MCP server with 30+ tools. | [Github](https://github.com/Q00/ouroboros) ![GitHub Repo stars](https://img.shields.io/github/stars/Q00/ouroboros?style=social) | Free |
