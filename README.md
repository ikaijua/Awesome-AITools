...
| OrcaReplay | Record/replay/fork debugger for coding agents. Captures below the harness so model traffic, shell exit codes, per-turn file changes and MCP calls share one timeline; replays a run offline byte-for-byte with the network off, or forks it from any checkpoint onto a different model. Works with Claude Code, Codex, opencode, Qwen Code, Cursor and others. | [URL](https://github.com/Continuum-AI-Corp/OrcaReplay) | Free (Apache-2.0) |
