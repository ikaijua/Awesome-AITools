...
| Trae Agent | 字节跳动开源的面向通用软件工程任务的 LLM 智能体，提供终端 CLI（`trae-cli`），能够理解自然语言指令并执行复杂软件工程工作流。支持 OpenAI、Anthropic、Gemini、OpenRouter、Ollama、豆包等多种模型，内置文件编辑、Bash 执行、顺序思考等工具，支持 MCP、轨迹记录和 Docker 模式运行。研究友好的模块化架构，便于扩展与分析 Agent 行为。[入门介绍](docs/trae-agent/README-CN.md) | [Github](https://github.com/bytedance/trae-agent) ![GitHub Repo stars](https://img.shields.io/github/stars/bytedance/trae-agent?style=social) | 免费 |
