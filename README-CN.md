...
| Trae Code | 🌱 字节跳动开源的面向通用软件工程任务的 LLM 智能体（原名 Trae Agent），提供终端 CLI（`trae-cli`），能够理解自然语言指令并执行复杂软件工程工作流。支持 OpenAI、Anthropic、Gemini、OpenRouter、Ollama、豆包等多种模型，内置文件编辑、Bash 执行、顺序思考等工具，支持 MCP、轨迹记录和 Docker 模式运行。研究友好的模块化架构，便于扩展与分析 Agent 行为。[入门介绍](docs/trae-code/README-CN.md) | [URL](https://www.trae.ai/) [Github](https://github.com/bytedance/trae-agent) ![GitHub Repo stars](https://img.shields.io/github/stars/bytedance/trae-agent?style=social) | 免费 |
...
| Trae Work | 🌱 字节跳动推出的 AI 工作助手，提供 Web、桌面和移动端工作空间。Code Mode 支持对话优先的云端编程任务委派与异步验收；Work Mode 面向研究、写作、规划、数据分析和文档生产等通用办公任务。支持与 Trae IDE/SOLO Mode 共享代码库索引、规则、自定义 Agent 和 MCP 能力，并支持并发 Cloud tasks。[入门介绍](docs/trae-work/README-CN.md) | [URL](https://www.trae.ai/) | 免费/付费 |
