...
| CarryCtx | 本地优先（Local-first）命令行工具，为编程 Agent 提供跨会话、跨窗口、跨 Git worktree 的持久化记忆：结构化任务、进度日志、决策记录，以及带 Git 感知的状态快照，全部存放在每个仓库的一个 SQLite 文件中。`carryctx resume` 可精确还原上一个 Agent 或窗口的进度；基于 SQLite FTS5 的全文搜索可按内容查找历史工作。默认不联网、不需要账号，可通过 CLI 或 MCP stdio 服务器接入任意 Agent（Claude Code、Codex、Copilot 等）。 | [Github](https://github.com/Xuepoo/carryctx) ![GitHub Repo stars](https://img.shields.io/github/stars/Xuepoo/carryctx?style=social) | 免费 |
