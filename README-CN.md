...
| MCP Lens | DeepSeek Harness 的开源插件，通过两个模型可见接口提供已配置的 MCP 工具：`mcp_search` 用于发现，`mcp_call` 用于调用。工具在被发现或调用前会应用明确的允许和拒绝过滤规则。 | [Github](https://github.com/labmimors/dsh-mcp-lens) ![GitHub Repo stars](https://img.shields.io/github/stars/labmimors/dsh-mcp-lens?style=social) | 免费 |
