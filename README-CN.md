...
|ClaudeMe|开源 Claude Code 分支，支持任意 OpenAI 兼容 API 直连国内大模型（DeepSeek、通义千问、Moonshot 等）。一个 JSON 文件配置多厂商多模型，内置个人 Wiki 知识库（LLM 实体提取 + 两阶段 RAG 查询），中文原生界面，默认零确认模式。基于 Bun 运行。|[Github](https://github.com/zrt-ai-lab/claudeme) ![GitHub Repo stars](https://img.shields.io/github/stars/zrt-ai-lab/claudeme?style=social)|免费|
